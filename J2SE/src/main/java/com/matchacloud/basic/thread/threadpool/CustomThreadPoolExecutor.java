 ...
 * <p>
 * 线程池概述：
 * 1. 创建线程方式之一，线程池维护着多个线程
 * 2. 等待着监督管理者分配可并发执行的任务。这避免了在处理短时间任务时创建与销毁线程的代价(创建线程对象 分配堆内存)
 * 3. 线程池不仅能够保证内核的充分利用，还能防止过分调度
 * 4. 线程池处理任务的场景类似：10个人去银行共5个窗口办理业务
 * 5. 线程池的线程对象可以复用的
 * <p>
 * 线程池优势：
 ...
 * <p>
 * 线程池架构：
 * 1. Java 中的线程池是通过Executor 框架实现的，该框架中用到了Executor->ExecutorService->ThreadPoolExecutor
 * 2. Executors(工具类) 这几个类
 * <p>
 * 注意事项：
 * 1. 实际开发自定义线程池，线程池不允许使用 Executors 去创建，因为阻塞队列太大可能堆溢出，而是通过 ThreadPoolExecutor 的方式
 * <p>
 * 线程池7大参数的解释以及合理设置参数
 ...
 * <p>
 * 线程池核心线程数 / 最大线程数设置(新):
 * 1. 核心/最大线程数写进nacos config 实现动态修改(因为业务高峰 低谷这个值需要变动)
 * 2. 监控线程池的负载 阻塞队列以及拒绝次数
 * 3. 根据线程池负载情况进行线程池调优
 * 4. 对于cpu密集任务：核心线程数=cpu核心数+1，最大线程数=cpu核心数+1
 * 5. 对于io密集型任务：核心线程数=最大线程数=cpu核心数*(1+等待时间/计算时间)
 * 一个任务耗时=等待时间(一个任务中等待io返回的时间)+计算时间(一个任务中内存计算时间)
 */
public class CustomThreadPoolExecutor {
     ...
     * <p>
     * 什么是核心线程：
     * 1.线程即使长时间没有在执行任务，也不会销毁线程对象的线程对象，除非手动设置
     * 2.核心线程对象会在线程池初始化或者有任务提交时被创建
     * 3.核心线程对象的存在使得线程池能够快速响应执行新任务，不用现创建线程对象
     * 4.优先执行任务且相对稳定
     */
    ...
    /**
     * 线程工厂：
     * 封装了线程对象创建的方式，线程池使用线程工厂，则该线程池会统一使用这种方法创建线程对象
     * <p>
     * 线程工厂有哪几种：
     * 1.自定义线程工厂：对于线程池里新建的线程进行线程名称、是否为守护线程、线程优先级等规定
     * 2.Executors.privilegedThreadFactory()
     * 3.Executors.defaultThreadFactory()，默认的，线程池使用该线程工厂该线程池里的线程都不是守护线程
     */
     ...
     * <p>
     * 线程池执行(工作)流程：
     * 一个线程池对象 核心线程数=5 一开始没创建核心线程对象
     * 新来任务 核心线程对象数<核心线程数 创建核心线程对象该对象执行任务
     * 如果核心线程对象数=核心线程数 即使核心线程存在空闲也加入阻塞队列
     * 入队失败创建非核心线程对象
     * 核心线程空闲了就会出队 如果队列空就阻塞挂起相当于还是空闲
     * 非核心线程数也满了，再来任务执行拒绝策略
     *
     * @param args
     */
