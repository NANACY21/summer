### juc概述
juc:java.util.concurrent 处理线程工具包

**进程/线程区别**  
`进程`:`操作系统`进行资源分配和调度的基本单位，指在系统中正在运行的一个应用程序，程序一旦运行就是进程,进程:资源分配的最小单位  
`线程`:系统分配处理器时间资源的基本单元，或者说进程之内独立执行的一个单元执行流。线程：程序执行的最小单位。 线程是任务调度和执行的基本单位
...
**wait()/sleep()区别**
都是暂停当前线程，当前线程不再占用cpu  
sleep()是 Thread 的静态方法，wait() 是 Object的方法  
sleep() 不会释放锁，它也不需要占用锁。wait 会释放锁，但调用它的前提是当前线程占有锁(即代码要在 synchronized 中)
它们都可以被 interrupted 方法中断

**用户线程/守护线程**
`用户线程`：主线程结束了，用户线程如果还在运行，则jvm存活
`守护线程`：例如做垃圾回收操作 如果jvm进程里没有用户线程了都是守护线程，则jvm结束

**Lock接口**  
Lock接口，需要手动加锁/释放锁 出现异常也不会自动释放锁 仍需要手动释放锁  
ReentrantLock类(可重入锁)是Lock接口的一个实现类 该类的一个对象作为锁，一个锁 对应 多个线程

多个线程执行同一个方法时为了保证该方法同一时刻只能被一个线程执行 这些线程执行方法时尝试获取的锁必须是同一个  
...
**线程间通信**  
wait()/notify() 可以暂停自己，让其他线程继续执行  
wait()/notify()如果不是来自同一个对象 会有问题 通信不成功 (此方法只应由作为此对象监视器的所有者的线程来调用)  
wait() :对于某一个参数的版本，实现中断和虛假唤醒是可能的，而且此方法应始终在循环中使用!!!因为wait()在哪里睡就在哪里醒 醒了之后应该再判断所以应while
...
**集合的线程安全**
集合类线程不安全的场景 有并发写 并发读 读写问题  
线程安全的vector 使用同步关键字 是悲观锁 性能可能不好  
CopyOnWriteArrayList线程安全 其底层原理：写时复制技术(类似git创建多个分支 同时修改 然后合并 乐观锁)  
hashset/hashmap线程不安全 解决方案：CopyOnWriteArraySet ConcurrentHashMap  
...
4. `死锁` 多个线程因为对方而阻塞 都执行不了
    验证死锁：jps:查当前运行的进程 进程号 jstack:jvm自带查看堆栈信息工具
...
2. 主存(运行内存，也叫主物理内存)
3. 外存(磁盘)
cpu运算数据是把主存数据复制到cpu和主存之间的高速缓存里来进行计算  
jvm规范中试图定义一种Java内存模型，来屏蔽掉各种硬件和操作系统的内存访问差异  
...
2. `原子性`:要求线程之间隔离性 互不干扰 主要表现为一个线程的写操作不被其他线程干扰导致写错误!!!保证原子性:synchronize关键字或原子类
3. `有序性`:程序编写的指令顺序和cpu最终运行的指令顺序可能不一致(指令重排序) 指令重排单线程永远没事 多线程可能有事!!!!
...
**volatile关键字**
被volatile修饰的变量：可见性、有序性(禁重排)  
可见性、有序性可以使用volatile关键字来解决  
