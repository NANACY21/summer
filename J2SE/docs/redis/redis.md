### Redis概述
1. `使用redis的好处` redis最常用于缓存 提升查询效率 减轻数据库的并发压力
2. `redis单线程` 当存在多个请求时 多个请求串行执行! 一个redis进程里只有一个处理请求的线程
   你（连接 A）、同事（连接 B）同时执行命令：一起跑，互不等待
   你在自己的客户端连续输两条命令：先执行第一条，结束再跑第二条
   总之 同一个redis连接下 命令是串行的
3. `redis工作机制`：
   1. 启动阶段：加载磁盘上的 RDB/AOF 文件，把数据全载入内存，加载完成后才接收请求
   2. 运行阶段：所有增删改查命令只操作内存，读写速度极快
   3. 持久化（落盘）：RDB、AOF 刷盘都是后台异步执行，不阻塞主线程、不影响正常内存读写
4. `redis内存数据持久化策略`：（两者可以同时开启，兼顾速度与数据安全 同时开启即有两种触发持久化的机制）
   1. RDB（快照）-> 典型定时同步：
      按配置的时间 + 数据变更阈值，周期性把全量内存数据生成快照存入磁盘。
      例：save 900 1 表示900 秒内有 1 条数据改动，就触发一次 RDB 落盘。
      后台子进程执行，不阻塞主线程。
   2. AOF（日志）-> 按策略刷盘，分三种（记录每一条写命令，追加到文件，刷盘规则可配置：）
      1. always：每条写命令执行完立刻刷盘（实时，性能最差，数据最安全）
      2. everysec（默认）：每秒批量刷盘（折中，最常用）
      3. no：交给操作系统自行决定刷盘时机（性能最好，丢数据风险略高）



### 缓存穿透
1. `含义`：数据在缓存和数据库中均不存在，造成数据库压力
   缓存穿透会增加mysql压力，数据库操作耗时从而影响系统响应时间
2. `解决方案`
   1. `缓存空对象`：当查询的数据在数据库中不存在时，也将一个空对象（如 null 或特定的空数据标识）缓存到 Redis 中，
      并设置一个较短的过期时间。这样下次相同的请求就可以直接从缓存中获取空对象，避免再次访问数据库
   2. `接口层校验`
      在接口层对请求参数进行严格的校验，过滤掉明显非法的请求，如请求的键格式不符合要求等。这样可以在请求到达缓存和数据库之前就将其拦截，减少不必要的查询
   3. `对于热点数据`：可以采用 “缓存永不过期” 的策略，即逻辑上不设置过期时间，而是在更新数据库时主动更新缓存。
   ...
   5. `分布式锁`
      相当于设置一个屏障 使得类似串行访问mysql 这起到了缓解数据库压力的作用。
   6. `布隆过滤器(不了解)`
...
### 缓存雪崩
1. `含义`：大量缓存数据在同一时间过期或失效，导致大量请求直接穿透缓存，涌向数据库，从而可能使数据库因压力过大而崩溃的情况
   缓存雪崩会使mysql压力过大
2. `解决方案`
   1. `避免必要的缓存数据消失`：在应用程序中定期主动更新缓存数据，而不是等到缓存过期后再去更新。
      保证缓存数据的及时性和一致性
   2. 避免一开始没有缓存数据 mysql会在一开始压力过大 预先写入缓存
      定时预热：设置定时任务，在业务低峰期定期对缓存进行预热。
      例如，每天凌晨 2 - 3 点，系统自动查询并更新第二天可能会用到的热点数据缓存，确保在业务高峰期来临时，缓存中有足够的数据可以响应请求
   3. 缓存过期时间加随机值 让每个key过期时间不一样
   4. redis高可用集群 避免redis宕机 导致mysql压力过大
   5. 增强数据库的抗压能力
      1. 数据库集群
...
### redis常见使用场景
1. `缓存`(使用redis做缓存 需要考虑`缓存一致性` 即数据库和redis数据一致 不能存在新旧数据情况)
   1. 缓存热点数据：如首页、列表数据 商品信息 适用于读多写少
      配套：设置过期时间、缓存更新 / 淘汰策略、防缓存穿透 / 击穿 / 雪崩
2. `计数器`
   利用 INCR/INCRBY 原子自增特性
   1. 文章阅读量、点赞数、访客数、接口调用次数、订单数量
   2. 接口限流 / 防刷 ：单 IP / 用户单位时间访问次数限制（如 1 分钟最多 100 次请求）
   3. 秒杀限流
      提前拦截超额请求，保护后端服务。
3. `分布式锁`
4. `利用过期特性`
...
5. `排行榜 / 有序集合（ZSet）`
   借助 ZSet 有序集合天然排序能力
   1. 直播间榜单、游戏战力榜、文章热度榜、积分排名、销量排行
6. `会话存储（Session 共享）`
   集群部署时，统一存放用户 Session、登录 token，实现多服务、多服务器会话共享，常见于 Web / 小程序 / APP 登录态
7. `秒杀场景`
...
1. 读数据库：
   1. 查 Redis 命中缓存直接返回
   2. 缓存无则查 DB -> 写入 Redis -> 返回
2. 写数据库：
   1. 更新 MySQL 数据库
   2. 删除 Redis 缓存 key
   3. 延时（500ms~1s）再次删除该 key（异步线程 / MQ）
3. 兜底：
   1. 定时全量 / 增量同步任务 矫正数据库、缓存数据一致
   2. 设置缓存过期时间
      即使更新逻辑出错，缓存到期自动失效，下一次查询加载最新数据，不会永久脏数据
   3. 要确保读写数据操作规范（Cache Aside 读逻辑）
      严格按照读写时数据库、缓存规定的操作顺序