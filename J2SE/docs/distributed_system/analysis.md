...
#### 性能监控概述
通过`性能监控`可以`诊断故障`、`调优`
常见性能监控工具：SkyWalking、Arthas、JVM命令、Linux命令、收集慢SQL


#### 分布式系统性能监控指标
1. CPU利用率
2. 内存使用率 是否内存占用过大 频繁fullgc风险
3. 请求耗时


#### 性能监控重点(面试最常问!!!)(分布式系统问题全流程闭环解决方案)
1. `监控环节`：
   `sql慢`、`接口慢`、`cpu高`
   这些问题是性能监控的重点
   要建立监控机制，一旦出现上述问题立刻监控到并告警，有一个控制台能展示(持久化)监控到的问题
   该控制台也可以定期统计问题次数是否变少。
2. `故障诊断+调优环节`：
   1. `sql慢`：
      可以结合业务/数据走势 分析慢的原因从而进行`explain分析、sql写法优化、索引优化、以及其他方面优化`
      [sql慢监控+诊断+调优](../mysql/analysis.md)
   2. `接口慢`：
      可能是系统故障(慢sql、fullgc、cpu高、网络异常、服务宕机、内存占用过高、连接池满了)或需要调优
      系统故障就解决相应问题，需要调优就结合实际场景优化(限流、熔断降级、同步改异步)
   3. `CPU高`：
      定位cpu高的线程(可能是业务线程或gc线程)
      如果业务线程arthas thread -n进一步定位代码位置，
      如果gc线程考虑是否fullgc，查看内存使用率，是否内存泄漏，`定位对应问题代码或问题类、问题对象实例`
      cpu高也可能不是jvm导致，要另外全局排查。
...
总结：cpu高排查命令：arthas thread -n、jstat，cpu高原因：死循环、内存泄漏等
...
1. `看问题出在哪`(涵盖代码优化、数据库优化)
   ...
   6. JVM：`jstack 进程PID` 该命令可以看到哪个线程阻塞/等待 在哪一行代码阻塞/等待 从而定位
      1. 卡在jdbc executequery 考虑有可能存在慢sql->没索引 索引失效 索引问题 explain分析 定位慢sql
...
7. arthas：watch trace jad dashboard thread(关注dashboard/jstat打印内容含义)
...
3. 数据库层面优化
   1. sql优化 索引优化
   2. `数据库读写分离`：将读操作和写操作分离到不同的数据库实例上，减轻主数据库的压力
   3. `数据库分库分表`：将数据分散存储在多个数据库或表中，提高数据库的并发处理能力
4. 架构层面优化
...
#### 接口总是超时怎么避免
看是 `需要解决故障` 还是 `需要调优`
先进行故障诊断(上面的接口超时诊断)，如果定位是问题：比如OOM、慢sql、FullGC、下游服务故障，解决问题
如果没有上述问题，就是超时，就需要进行响应的调优：
1. 客户端优化：
   1. 合理设置超时时间，不要无限等待
   2. 超时重试(仅针对幂等接口) + 熔断 防止大量超时压垮下游
   3. 限流
   4. 请求连接池优化 调大最大连接数、空闲连接存活时间 定时清理无效空闲连接 避免连接耗尽导致握手超时
2. 服务端优化：
   1. 慢sql优化
   2. 使用缓存
   3. 数据库配置 调大连接池
   4. 耗时任务走异步 分析代码逻辑是否导致慢
   5. 线程池满直接拒绝 不堆积请求导致排队超时
3. 网络优化：
   1. 跨机房 / 跨公网接口延迟高：内网专线、调整 DNS、使用长连接
   2. 防火墙 / 网关限制超时时间：Nginx、SLB 默认 60s 超时，长任务需调大
   3. RPC 框架超时（Dubbo/GRPC）
      1. 避免请求到坏机器
      2. 开启负载均衡轮询 / 最小活跃数，分散压力
4. 架构层面：彻底规避超时（高并发系统）
   1. 预计算热点数据：定时任务提前统计报表、排行榜，接口直接读结果
   2. 接口拆分：大接口拆分成多个轻量小接口，前端并行请求，单次响应极快
   3. 平时监控告警：
      监控指标：接口平均耗时、95 分位耗时、超时次数、线程池队列堆积、慢 SQL，提前发现性能瓶颈，不等大量超时发生。
