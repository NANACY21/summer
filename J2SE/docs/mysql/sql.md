...
### 参数相关
1. 查全局默认存储引擎 SHOW VARIABLES LIKE 'default_storage_engine';
2. 查当前事务隔离级别(全局) SELECT @@global.transaction_isolation;
3. 查看MySQL默认编码格式 `show variables like 'character%';`
4. 查看MySQL允许的最大连接数 `show variables like '%max_connections%';`