...
**索引设计原则**
1. 哪些字段适合/不适合加索引!!!:去重 得排序 所以加索引好(表数据量小 且对某字段去重查询操作频繁 给该字段加索引合适)
   去重字段如果不加索引，会把要查询的数据全查到内存进行一一比对去重 如果加索引 遍历索引即可挑选出 去重数据
2. (重点)索引失效的场景:
...
**并发事务引发的问题(重点)**
1. `脏写`(Dirty Write 丢失修改) (InnoDB任何隔离级别 永远不存在脏写)
   一开始100 事务1改成200未提交 此时事务2改成300 现在事务1回滚 数据变成100 事务2写失败->脏写，由于存在排他锁，所以脏写在innodb引擎不会出现  
2. `脏读`：`读到未生效的数据` 事务a没提交的数据被另一个事务读到 后来事务a回滚
3. `不可重复读`：T1 开启事务，第一次查询 money=100 T2 修改 money=200 并提交；T1 再次查询变成 200，同一事务两次读取不一致
4. `幻读`：T1：select * from user where id>10 -> 查出 3 条；T2：insert 一条 id=11 并 commit；T1 再次同 SQL 查出 4 条，凭空多一条 = 幻读。
...
**事务隔离性**
**(重点)事务4个隔离级别(一个全局或会话变量)：当多个事务并发访问数据库时，这些事务之间要有所隔离，即互不干扰：**
1. `读取未提交`： 允许读没提交的，设置该隔离级别`脏读`、`不可重复读`、`幻读`依然会存在
2. `读取已提交`： 允许读已提交的，设置该隔离级别 `脏读`问题无、`不可重复读`、`幻读`依然会存在
3. `可重复读`：(mysql默认级别  mysql该级别并发问题都解决了!!!) 设置这个隔离级别 `脏读` `不可重复读`不会有了 如果该级别想避免`幻读`需要手动加锁!!!
